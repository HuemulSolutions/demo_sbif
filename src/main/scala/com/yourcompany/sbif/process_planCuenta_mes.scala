...
object process_planCuenta_mes {
  ...
  */
  def main(args : Array[String]) {
    //Creacion API
    val huemulBigDataGov  = new huemul_BigDataGovernance(s"Masterizacion tabla tbl_comun_planCuenta_mes - ${this.getClass.getSimpleName}", args, globalSettings.Global)
  ...
  def procesa_master(huemulBigDataGov: huemul_BigDataGovernance, ControlParent: huemul_Control, param_ano: Integer, param_mes: Integer): Boolean = {
    ...
    try {             
      ...
      /*************** LOGICAS DE NEGOCIO **********************/
      /*********************************************************/
      //instancia de clase tbl_comun_planCuenta_mes 
      val huemulTable = new tbl_sbif_planCuenta_mes(huemulBigDataGov, Control)
      
      Control.NewStep("Generar Logica de Negocio")
      huemulTable.DF_from_SQL("FinalRAW"
                          , s"""SELECT DISTINCT TO_DATE("${param_ano}-${param_mes}-1") as periodo_mes
                                     ,ano
                                     ,mes
                                     ,planCuenta_id
                                     ,planCuenta_Nombre
...
*/
object process_planCuenta_mes_Migrar {
 
 def main(args : Array[String]) {
   //Creacion API
    val huemulBigDataGov  = new huemul_BigDataGovernance(s"Migración de datos tabla tbl_comun_planCuenta_mes  - ${this.getClass.getSimpleName}", args, globalSettings.Global)
