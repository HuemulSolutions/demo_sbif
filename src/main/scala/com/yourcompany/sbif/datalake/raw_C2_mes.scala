 ...
 */
class raw_C2_mes(huemulBigDataGov: huemul_BigDataGovernance, Control: huemul_Control) extends huemul_DataLake(huemulBigDataGov, Control) with Serializable  {
   this.LogicalName = "sbif_C2_mes"
   ...
   CurrentSetting.FileName = "c2{{YYYY}}{{MM}}{{Cod_Banco}}.txt"
   ...
   //Forma detallada
   CurrentSetting.DataSchemaConf.AddColumns("planCuenta_id", "planCuenta_id", StringType, "Código contable. Es un campo de 7 digitos que identifica el concepto contable que se describe en el archivo Modelo-MC2.txt.")
   CurrentSetting.DataSchemaConf.AddColumns("c2_Monto", "monto", DecimalType(20,2), "Total (Valor en millones de pesos chilenos)")
...
object raw_C2_mes_test {
  ...
  def main(args : Array[String]) {
    ...
    val DF_RAW =  new raw_C2_mes(huemulBigDataGov, Control)
